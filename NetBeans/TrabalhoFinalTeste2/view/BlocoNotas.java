...
public class BlocoNotas extends javax.swing.JFrame {
    ...
    private void jmiAbrirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmiAbrirActionPerformed
        ...
        if (resultado == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                Serializacao serializacao = arquivo.abrir(arquivoSelecionado.getAbsolutePath());
                jTextPane1.setText(serializacao.getTexto());  // Definir o conteúdo do editor

                //String texto = ArquivoBinario.abrir(arquivoSelecionado.getAbsolutePath());
                //jTextPane1.setText(texto);  // Definir o conteúdo do editor
                aplicarFormatacoes(serializacao.getFormatações());
    ...
    private void jmiSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmiSalvarActionPerformed
        ...
        if (resultado == JFileChooser.APPROVE_OPTION) {
            ...
            if (arquivo.existeArquivo(arquivoSelecionado.getAbsolutePath())) {
             ...
             // Criar uma lista de formatações
            List<String> formatações = new ArrayList<>();
            if (jTextPane1.getFont().isBold()) {
                formatações.add("Negrito");
            }
            if (jTextPane1.getFont().isItalic()) {
                formatações.add("Itálico");
            }
            if (jTextPane1.getFont().getSize() > 12) {  // exemplo: maior que 12px indica formatação especial
                formatações.add("grande");
            }

            // Salvar texto e formatação
            Serializacao serializacao = new Serializacao(jTextPane1.getText(), formatações);
            
            
            try {
                arquivo.salvar(arquivoSelecionado.getAbsolutePath(), serializacao);
            } catch (IOException e) {
    ...
    private void aplicarFormatacoes(List<String> formatações) {
        // Aplicar formatações no texto
        StyledDocument doc = jTextPane1.getStyledDocument();
        SimpleAttributeSet atributos = new SimpleAttributeSet();

        for (String formato : formatações) {
            switch (formato) {
                case "Negrito":
                    StyleConstants.setBold(atributos, true);
                    break;
                case "Itálico":
                    ...
                    break;
                case "grande":
                    StyleConstants.setFontSize(atributos, 18);
                    break;
            }
        }

        doc.setCharacterAttributes(0, jTextPane1.getText().length(), atributos, false);
    }
